...
const ActionExecutor: React.FC<ActionExecutorProps> = ({ action, apiBaseUrl }) => {
  ...
  if (result.status === 'pending') {
    return (
      <div className="my-4 p-2 bg-blue-900/30 border border-blue-700 rounded">
        <div className="flex items-center space-x-3">
          <div className="animate-spin h-5 w-5 border-2 border-blue-500 rounded-full border-t-transparent"></div>
          <div>
            <p className="text-blue-300 font-semibold">Executing Action: {action.type}</p>
            <p className="text-primary text-sm">{result.message}</p>
          </div>
        </div>
      </div>
    );
  ...
  if (result.status === 'success') {
    return (
      <div className="my-4 p-2 bg-emerald-900/30 border border-emerald-700 rounded">
          ...
          <div className="flex-1">
            ...
            {result.result && (
              <div className="mt-3 p-3 bg-slate-950 rounded border border-border">
                <p className="text-xs text-text-tertiary uppercase font-semibold mb-2">Result:</p>
  ...
  // Error state
  return (
    <div className="my-4 p-2 bg-red-900/30 border border-red-700 rounded">
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1">
          ...
          {result.traceback && (
            <details className="mt-3">
              ...
              <pre className="mt-2 p-3 bg-slate-950 rounded border border-border text-xs text-error overflow-x-auto">
                {result.traceback}
              </pre>
            </details>
          )}
