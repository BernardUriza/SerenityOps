...
const CVJobProgress: React.FC<CVJobProgressProps> = ({ onComplete, onError }) => {
  ...
  if (!job) {
    return (
      <div className="p-2 bg-macPanel/70 backdrop-blur-md rounded-mac border border-macBorder/40 text-macSubtext text-sm shadow-[0_2px_6px_rgba(0,0,0,0.2)]">
  ...
  if (job.status === 'queued') {
    return (
      <div className="p-2 bg-blue-900/30 border border-blue-700 rounded-mac backdrop-blur-md shadow-[0_2px_6px_rgba(0,0,0,0.2)]">
        <div className="flex items-center space-x-3">
          <div className="animate-spin h-5 w-5 border-2 border-blue-500 rounded-full border-t-transparent"></div>
          <div>
            <p className="text-blue-300 font-semibold">Queued</p>
            <p className="text-macAccent text-sm">Waiting to start...</p>
          </div>
        </div>
      </div>
    );
  ...
  if (job.status === 'running') {
    return (
      <div className="p-2 bg-macPanel/70 backdrop-blur-md rounded-mac border border-macBorder/40 shadow-[0_2px_6px_rgba(0,0,0,0.2)]">
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-macText">{job.stage}</p>
            <span className="text-xs text-macSubtext">{job.progress}%</span>
          </div>

          {/* Animated progress bar */}
          <div className="w-full bg-macPanel/70 rounded-full h-2 overflow-hidden">
            <motion.div
              className="bg-macAccent h-2"
              initial={{ width: 0 }}
          ...
          <div className="flex items-center space-x-2">
            <div className="animate-pulse h-2 w-2 bg-macAccent rounded-full"></div>
            <p className="text-xs text-macSubtext">Generating your CV...</p>
          </div>
        </div>
      </div>
    );
  ...
  if (job.status === 'success') {
    return (
      <div className="p-2 bg-emerald-900/30 border border-emerald-700 rounded-mac backdrop-blur-md shadow-[0_2px_6px_rgba(0,0,0,0.2)]">
        <div className="space-y-4">
          ...
          {job.download_url && (
            <div className="mt-3 p-3 bg-macBg rounded-mac border border-macBorder/40 backdrop-blur-md shadow-[0_2px_6px_rgba(0,0,0,0.2)]">
              ...
              {job.filename && (
                <div className="mt-2 text-xs text-macSubtext">
          ...
          <button
            onClick={clearJob}
            className="text-xs text-emerald-500 hover:text-success transition-all duration-300 ease-mac"
          >
            Clear
          </button>
        </div>
  ...
  if (job.status === 'error') {
    return (
      <div className="p-2 bg-red-900/30 border border-red-700 rounded-mac backdrop-blur-md shadow-[0_2px_6px_rgba(0,0,0,0.2)]">
        <div className="space-y-4">
          <div className="flex items-start space-x-3">
          ...
          {job.error_message && (
            <details className="mt-2">
              ...
              <pre className="mt-2 p-3 bg-macBg rounded-mac border border-macBorder/40 text-xs text-error overflow-x-auto max-h-40">
                {job.error_message}
              </pre>
            </details>
          )}
          ...
          <button
            onClick={clearJob}
            className="text-xs text-red-500 hover:text-error transition-all duration-300 ease-mac"
          >
            Clear
          </button>
        </div>
